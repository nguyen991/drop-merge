...
namespace Databrain.UI
{
    ...
    public class DatabrainAssetInspectorWindow : EditorWindow
    {

        [MenuItem("Tools/Databrain/Debug/Asset Inspector", false, 300)]
        static void Init()
        {
            DatabrainAssetInspectorWindow editor = (DatabrainAssetInspectorWindow)EditorWindow.GetWindow(typeof(DatabrainAssetInspectorWindow));
        ...
        void DrawAssets()
        {
            ...
            if (GUILayout.Button("Fix missing/null sub-assets"))
            {    
                var _dialog = EditorUtility.DisplayDialog("Fix missing sub-assets", "This will delete all null sub-assets inside of a DataLibrary.\nPlease ignore possible warning messages afterwards.\nDo you want to proceed?", "Yes", "No");
                if (_dialog)
                {
                    AssetDatabaseHelper.FixMissingScripts(activeObject);
                }
            }
