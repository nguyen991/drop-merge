...
namespace Databrain.Logic
{
    ...
    public class LogicControlleEditor : Editor
    {
                    ...
                    for (int s = 0; s < _sceneObjects.Count; s++)
                    {
                        ...
                        while (iterator.NextVisible(enterChildren))
                        {
                            ...
                            if (iterator.propertyPath == "exposedReferences")
                            {
                                ...
                                //#############
                                // Commented out because it's not working correctly
                                //#############
                                // Clean up list
                                ...
                                for (int i = 0; i < iterator.arraySize; i++)
                                {
                                    ...
                                    if (_tt == null)
                                    {
                                        // Try getting the type without assembly qualified name
                                        var _splitted = _type.stringValue.Split(new[] { ',' }, 2);
                                        _tt = System.Type.GetType(_splitted[0]);
